...
import { registerWithEmailPassword, signInWithEmailPassword, signOutUser } from './services/firebase.js'
...
function App() {
  ...
  const renderPage = () => {
    if (page === 'login') {
      return <LoginHomepage onContinue={handleLogin} onCreateAccount={() => setPage('register')} onGoogleAccount={() => setPage('home')} />
    }
    if (page === 'register') {
      return <RegisterForm onSubmit={handleRegister} onBack={() => setPage('login')} onGoogleAccount={() => setPage('home')} />
    }
